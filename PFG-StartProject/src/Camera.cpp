...
Camera::Camera()
{
	_cameraPos = glm::vec3(0.0f, 5.0f, 7.5f);
	_initTarget = glm::vec3(0.0f, 0.0f, -1.0f);
	...
	oldMousePosition = glm::vec2(400, 400);

	_up = glm::vec3(0, 1, 0); 

	//Build  projection and viewing matrices
	_projMatrix = glm::infinitePerspective(45.0f, 1.0f, 0.1f);
	_viewMatrix = glm::lookAt(_cameraPos, _initTarget, _up);
...
void Camera::Update(Input* input)
{
	...
	direction = glm::vec3(cos(_cameraAngleY) * sin(_cameraAngleX), sin(_cameraAngleY), cos(_cameraAngleY) * cos(_cameraAngleX));
